...
2026-01-17 03:03:45 - Исправлен парсинг секции mcp.pg_server в config.yaml
### Обоснование
* load_config присваивал dict в поле MCPConfig.pg_server, что ломало доступ к атрибутам
### Детали реализации
* Добавлен маппинг вложенного словаря на MCPPostgresServer при разборе YAML
* Обновлены тесты на корректную типизацию вложенной конфигурации
...
## Решение: Docker setup для кроссплатформенности
2026-01-12 19:28:42 - Решение: Docker setup для кроссплатформенности
### Обоснование
* Необходимость унифицированной среды выполнения для устранения проблем ОС-специфичности (Linux/Windows)
* Обеспечение консистентности между разработкой и продакшеном
* Упрощение развертывания и масштабирования
### Детали реализации
* Создание Dockerfile с Python 3.11 для приложения MaymunAI
* Настройка docker-compose.yml с PostgreSQL 15 и volumes для данных
* Конфигурация environment переменных для API ключей и БД
* Адаптация Linux скриптов для работы внутри контейнера
* Использование volumes для персистентности данных PostgreSQL
